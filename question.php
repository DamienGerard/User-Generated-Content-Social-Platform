...
<?php
    ...
    if (isset($_GET['content_id'])) {
        ...
        $query = "SELECT content.title, content.date, question.description, user.user_id, user.user_name, user.f_name, user.l_name, user.profile_pic FROM webprojectdatabase.content INNER JOIN webprojectdatabase.question ON content.content_id = question.question_id INNER JOIN webprojectdatabase.user ON content.user_id = user.user_id WHERE content.marked=0 AND content.content_id=".$content_id;
